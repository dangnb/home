...
public class CreateInboundTransactionCommandHandlerTests
{
    private readonly Mock<IInventoryTransactionRepository> _mockTransactionRepo;
    private readonly Mock<IProductRepository> _mockProductRepo;
    private readonly Mock<IUnitOfWork> _mockUnitOfWork;
    private readonly CreateInboundTransactionCommandHandler _handler;

    public CreateInboundTransactionCommandHandlerTests()
    {
        _mockTransactionRepo = new Mock<IInventoryTransactionRepository>();
        _mockProductRepo = new Mock<IProductRepository>();
        _mockUnitOfWork = new Mock<IUnitOfWork>();

        _handler = new CreateInboundTransactionCommandHandler(
    ...
    {
        // Arrange
        var command = new CreateInboundTransactionCommand("REF-001", "Notes", new List<TransactionLineDto>());
    ...
    public async Task Handle_WhenProductDoesNotExist_ShouldThrowArgumentException()
    {
        ...
        var command = new CreateInboundTransactionCommand("REF-001", "Notes", new List<TransactionLineDto>
        {
            new TransactionLineDto(productId, 10, 100)
        });
    ...
    public async Task Handle_WhenValid_ShouldCreateTransaction()
    {
        ...
        capturedTransaction.Type.Should().Be(TransactionType.Inbound);
        capturedTransaction.Lines.Should().HaveCount(1);
        capturedTransaction.Lines.First().ProductId.Should().Be(productId);
        capturedTransaction.Lines.First().Quantity.Should().Be(10);
